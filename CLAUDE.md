...
1. **Carte** — Interactive D3.js SVG map with zoom/pan, department/region layers, and insets for Île-de-France and overseas territories (DROMs)
...
Static data lives in `src/data/` (101 departments, 18 regions, adjacency graph, DROM coordinates). GeoJSON geometry is loaded lazily via `useGeoData` hook from `src/geo/`.
...
The D3 map (`CarteFrance.tsx`) uses `useEffect` for zoom behavior and `geoPath` rendering. Inset territories use `GroupInset` for coordinate transforms.
